...
// "불닭 돈까스"는 하나의 메뉴임 => 단순한 정보일 뿐 복잡한 정보는 필요없음

// 2. 식당에서 요리하는 요리사 입장에서
// "불닭 돈까스"는 하나의 음식임 => 요리이므로 복잡한 정보가 필요함

// 3. 식당에서 고객과 요리사를 관리하는 주인의 입장에서
// "불닭 돈까스"는 하나의 상품임 => 하나의 사업이므로 식당운영에 필요한 정보 필요
...
// 하지만, 기본은 지켜가면서 요리사의 역량에 따라 제각각의 결과물을 만들어 냄
// 어느 음식점을 가더라도 조금의 차이는 존재하지만 "불닭 돈까스"라는 틀은 같음. =인터페이스

// 기본 base클래스
// 인터페이스를 구현하면 자식클래스에는 추상메서드를 반드시 재정의해야함
// 만일. 재정의하고 싶지 않거나, 재정의할 필요가 없는데도 재정의해야하는 불편함이 존재할 수 있음.
// => 이럴 경우 추상클래스를 통해 임의로 구현한 다음
// 이 추상클래스를 상속해서 클래스를 만드는 것이 편리함
// 이때 만든 클래스를 기본 클래스라 함.
